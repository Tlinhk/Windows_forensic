...
class AddEvidenceWizard(QDialog):
    ...
    def start_volatile_collection(self):
        ...
        try:
            ...
            if main_window:
                ...
                if (
                    hasattr(main_window, "menu_btns_list")
                    and hasattr(main_window, "volatile_btn")
                    and main_window.volatile_btn in main_window.menu_btns_list
                ):
                    title, factory = main_window.menu_btns_list[
                        main_window.volatile_btn
                    ]
                ...
                QMessageBox.warning(
                    self,
                    "Error",
                    "Could not find main window to switch to volatile collection",
                )

        except Exception as e:
            print(f"Error starting volatile collection: {e}")
            QMessageBox.warning(
                self, "Error", f"Failed to start volatile collection: {e}"
            )

    def on_volatile_collection_complete(self, success, message, package_path):
        """Called when volatile collection is complete"""
        print(
            f"on_volatile_collection_complete called with success={success}, message={message}, path={package_path}"
        )
        ...
        if success and package_path:
            ...
            self.collection_state["output_path"] = (
                "E:/ForensicCollection/volatile_collection/"
            )
        ...
        print(
            f"Volatile collection completed: {message}, wizard shown with completion status"
        )
            ...
            if hasattr(self, "volatile_page"):
                ...
                if hasattr(volatile_page, "collection_finished"):
                    ...
                    def patched_collection_finished(success, message, package_path):
                        print(
                            f"Collection finished detected via monkey patch: {success}, {message}, {package_path}"
                        )
    ...
    def wizard_collection_finished(
        self, collection_type, success, message, package_path
    ):
        """Method to be called directly from collection pages"""
        print(
            f"wizard_collection_finished called: {collection_type}, {success}, {message}, {package_path}"
        )
    ...
    def check_collection_status(self):
        ...
        try:
            ...
            if (
                hasattr(self, "volatile_page")
                and self.collection_state["volatile_started"]
                and not self.collection_state["volatile_completed"]
            ):

                volatile_page = self.volatile_page

                # Check if collection is finished by looking at UI state AND collection actually happened
                if hasattr(volatile_page, "ui") and hasattr(
                    volatile_page.ui, "startCollectionBtn"
                ):
                    ...
                    if hasattr(volatile_page.ui, "evidenceLogText"):
                        ...
                        evidence_log_has_content = (
                            "SHA-256:" in log_content
                            or "Package:" in log_content
                            or "Collection completed:" in log_content
                        )
                    ...
                    if button_enabled and evidence_log_has_content:
                        ...
                        if hasattr(volatile_page.ui, "outputPathEdit"):
                            output_path = (
                                volatile_page.ui.outputPathEdit.text() or output_path
                            )
            ...
            if (
                hasattr(self, "nonvolatile_page")
                and not self.collection_state["nonvolatile_completed"]
            ):
                nonvolatile_page = self.nonvolatile_page
                # Check if collection is finished by looking at UI state
                if (
                    hasattr(nonvolatile_page, "ui")
                    and hasattr(nonvolatile_page.ui, "startCollectionBtn")
                    and nonvolatile_page.ui.startCollectionBtn.isEnabled()
                ):
                    # Collection finished, button is enabled again
                    self.collection_check_timer.stop()
                    ...
                    if hasattr(nonvolatile_page.ui, "outputPathEdit"):
                        output_path = (
                            nonvolatile_page.ui.outputPathEdit.text() or output_path
                        )
            ...
            if main_window:
                ...
                if (
                    ...
                    if hasattr(nonvolatile_page, "set_case_data") and self.case_id:
                        ...
                        if case_info:
                            case_data = {
                                "case_id": case_info.get(
                                    "case_code", f"CASE-{self.case_id}"
                                ),
                                "case_name": case_info.get("title", "Unknown Case"),
                                "investigator": "Current User",
                ...
                QMessageBox.warning(
                    self,
                    "Error",
                    "Could not find main window to switch to non-volatile collection",
                )

        except Exception as e:
            print(f"Error starting non-volatile collection: {e}")
            QMessageBox.warning(
                self, "Error", f"Failed to start non-volatile collection: {e}"
            )

    def add_files(self):
        """Add files to the list for import mode"""
        file_paths, _ = QFileDialog.getOpenFileNames(
            self, "Select Evidence Files", "", "All Files (*.*)"
        )
    ...
    def update_collection_status(self, volatile_done=False, nonvolatile_done=False):
        ...
        if volatile_done:
            output_path = self.collection_state.get(
                "output_path", "E:/Evidence/volatile_collection/"
            )
            package_name = (
        ...
        if nonvolatile_done:
            output_path = self.collection_state.get(
                "output_path", "E:/Evidence/nonvolatile_collection/"
            )
            package_name = (
                os.path.basename(output_path)
                if output_path
    ...
    def get_wizard_data(self):
        ...
        data = {
            ...
            "evidence_type": (
                "volatile" if self.ui.volatileTypeRadio.isChecked() else "nonvolatile"
            ),
    ...
    def add_evidence_to_database(self, evidence_data):
        ...
        try:
            ...
            if evidence_data["mode"] == "import":
                ...
                for file_path in evidence_data["files"]:
                    ...
                    if artifact_id and evidence_data.get("calculate_hash", False):
                        if os.path.isfile(file_path):
                            print(
                                f"DEBUG Wizard: Calculating hash for {filename} ({self.format_file_size(file_size)})"
                            )

                            # Use thread-based hash calculation for large files
                            if file_size > 100 * 1024 * 1024:  # > 100MB - show progress
                                hash_value = self.calculate_file_hash_with_progress(
                                    file_path, filename
                                )
                            else:
                ...
                collection_name = (
                    f"Collection_{evidence_data['evidence_type']}_{self.case_id}"
                )
    ...
    def finalize_evidence_addition(self, wizard_data):
        ...
        if evidence_records:
            ...
            QMessageBox.critical(
                self, "Error", "Failed to add evidence to database. Please try again."
            )
